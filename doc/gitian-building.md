...
Clone the git repositories for FBTC and Gitian.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/FBTC/.git/
...
For example:
```bash
URL=https://github.com/akx20000a/FBTC.git
COMMIT=d5522ed66a924a2d505fe45ed2487e741c6c0917
./bin/gbuild --commit FBTC=${COMMIT} --url FBTC=${URL} ../FBTC/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit FBTC=${COMMIT} --url FBTC=${URL} ../FBTC/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit FBTC=${COMMIT} --url FBTC=${URL} ../FBTC/contrib/gitian-descriptors/gitian-osx.yml
```
...
BTCPATH=/some/root/path/FBTC
SIGPATH=/some/root/path/foxbtc-detached-sigs

./bin/gbuild --url FBTC=${BTCPATH},signature=${SIGPATH} ../FBTC/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/foxbtc-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/foxbtc-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/foxbtc-osx-build.assert
```
...
[FBTC/gitian.sigs](https://github.com/FBTC/gitian.sigs/) repository, or if that's not possible create a pull
request.
