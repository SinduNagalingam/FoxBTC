...
public:
    CMainParams() {
        ...
        consensus.FBTCHeight = 491407; // Around 10/25/2017 12:00 UTC
        consensus.FBTCPremineWindow = 8000;
        consensus.FBTCZawyLWMAHeight = 536200; // Around 07/01/2018
        consensus.FBTCEquihashForkHeight = 536200; // Around 07/01/2018
        consensus.FBTCPremineEnforceWhitelist = true;
        ...
        consensus.FBTCMaxFutureBlockTime = 12 * 10 * 60; // 120 mins
        ...
        nDefaultPort = 8336; // different port than Bitcoin
        ...
        vSeeds.emplace_back("167.99.145.155:8336", true);
        vSeeds.emplace_back("206.81.15.128:8336", true);
...
public:
    CTestNetParams() {
        ...
        consensus.FBTCHeight = 1;
        consensus.FBTCZawyLWMAHeight = -1; // Activated on testnet
        consensus.FBTCEquihashForkHeight = 14300;
        consensus.FBTCPremineWindow = 50;
        consensus.FBTCPremineEnforceWhitelist = false;
        consensus.powLimit = uint256S("0007ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff");
        ...
        consensus.FBTCMaxFutureBlockTime = 7 * 10 * 60; // 70 mins
        ...
        vSeeds.emplace_back("eu-test-dnsseed.foxbtc-official.org", true);
        vSeeds.emplace_back("test-dnsseed.foxbtc.org", true);
...
public:
    CRegTestParams() {
        ...
        consensus.FBTCHeight = 2000;
        consensus.FBTCZawyLWMAHeight = -1; // Activated on regtest
        consensus.FBTCEquihashForkHeight = 2001;
        consensus.FBTCPremineWindow = 10;
        consensus.FBTCPremineEnforceWhitelist = false;
...
bool CChainParams::IsPremineAddressScript(const CScript& scriptPubKey, uint32_t height) const {
    ...
    assert((uint32_t)consensus.FBTCHeight <= height &&
           height < (uint32_t)(consensus.FBTCHeight + consensus.FBTCPremineWindow));
    int block = height - consensus.FBTCHeight;
    int num_unlocked = consensus.FBTCPremineWindow * 40 / 100;  // 40% unlocked.
    int num_locked = consensus.FBTCPremineWindow - num_unlocked;  // 60% time-locked.
    ...
    if (block < num_unlocked) {
        ...
        int lock_time = consensus.FBTCHeight + stage_lock_time * (1 + stage);
