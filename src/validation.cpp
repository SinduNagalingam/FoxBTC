...
bool static IsFBTCHardForkEnabled(int nHeight, const Consensus::Params& params) {
    return nHeight >= params.FBTCHeight;
}

bool IsFBTCHardForkEnabled(const CBlockIndex* pindexPrev, const Consensus::Params& params) {
    if (pindexPrev == nullptr) {
        return false;
    }

    return IsFBTCHardForkEnabled(pindexPrev->nHeight, params);
}

bool IsFBTCHardForkEnabledForCurrentBlock(const Consensus::Params& params) {
    AssertLockHeld(cs_main);
    return IsFBTCHardForkEnabled(chainActive.Tip(), params);
}
...
bool ReadBlockFromDisk(CBlock& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams)
{
    ...
    bool postfork = block.nHeight >= (uint32_t)consensusParams.FBTCHeight;
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    if (IsFBTCHardForkEnabled(pindex->pprev, consensusparams)) {
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (block.nHeight < (uint32_t)consensusParams.FBTCHeight) {
...
static bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check block height for blocks after FBTC fork.
    if (nHeight >= consensusParams.FBTCHeight && block.nHeight != (uint32_t)nHeight)
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + std::min(consensusParams.FBTCMaxFutureBlockTime,
...
static bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    if (nHeight >= consensusParams.FBTCHeight &&
        nHeight < consensusParams.FBTCHeight + consensusParams.FBTCPremineWindow &&
        consensusParams.FBTCPremineEnforceWhitelist)
    {
